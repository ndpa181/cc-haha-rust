...
This project includes a set of refactored modules implementing a **decoupled architecture** for task orchestration, memory, and reporting.
...
│   ├── dag.py               # TaskDAG - directed acyclic graph scheduling
│   ├── orchestrator.py      # Orchestrator - high-level workflow orchestration
│   └── visualize.py         # DAGVisualizer - visualization export
├── memory/                   # Memory system
│   └── knowledge_graph.py   # KnowledgeGraph - persistent knowledge graph
└── exploration/              # Report generation
    └── report_builder.py     # ReportBuilder - structured reports with diagrams

src-rust/                     # Rust execution layer
├── tool-executor/           # Process execution (memory/timeout limits)
├── sandbox/                 # seccomp syscall filtering
├── vcr/                     # API response recording/playback
└── verification/             # Adversarial verification framework

src/integration/              # TypeScript integration layer
├── taskDagWrapper.ts       # TaskDAG TypeScript wrapper
├── knowledgeGraphWrapper.ts # KnowledgeGraph TypeScript wrapper
├── reportBuilderWrapper.ts  # ReportBuilder TypeScript wrapper
└── adapters.ts            # Existing codebase adapters
```

### Core Modules

#### TaskDAG - Task Scheduling

Directed acyclic graph with topological sort and parallel execution:
...
# Get execution order (Kahn's algorithm)
...
#### KnowledgeGraph - Memory System

Persistent knowledge graph with weighted search and relationships:
...
# Weighted search (name 3x > description 2x > content 1x)
...
Structured reports with Mermaid diagrams:
...
High-level orchestrator combining DAG, Memory, and Report:
...
Orchestrator.execute()
    │
    ├─► TaskDAG.getExecutionOrder()  # Topological sort
    │       │
    │       ▼
